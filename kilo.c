...
void editorUpdateSyntax(erow *row) {
    ...
    char **keywords = E.syntax->keywords; // make keywords an alias for E.syntax->keywords
...
void editorFindCallback(char *query, int key) {
    ...
    if (key == '\r' || key == '\x1b') {
        ...
        direction = 1; // for searching forward
    } else if (key == ARROW_LEFT || key == ARROW_UP) {
        direction = -1; // for searching backward
    } else {
        last_match = -1;
        direction = 1;
    }
    ...
    for (i = 0; i < E.numrows; i++) {
        current += direction;
        // make current go from the end back to the beginning of the file, or vice versa
