...
int getWindowSize(int *rows, int *cols) {
    struct winsize ws;
    // ioctl = Input/Output Control Get Window Size
...
void editorRefreshScreen() {
    write(STDOUT_FILENO, "\x1b[2J", 4); // write 4 bytes in the terminal
    write(STDOUT_FILENO, "\x1b[H", 3);
    editorDrawRows();
    write(STDOUT_FILENO, "\x1b[H", 3);
}
...
/*** init ***/
void initEditor() {
    // add screenrows and screencols to global editor state
